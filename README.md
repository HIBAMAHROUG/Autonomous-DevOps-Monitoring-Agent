...
les tests utilisent les chemins de compatibilité configurés par `conftest.py`.
...
>  **CRITIQUE :** si votre dépôt public contient un fichier `kube-docker-config.yaml` avec des certificats client et des clés privées embarqués, ce credential doit être considéré comme compromis.
